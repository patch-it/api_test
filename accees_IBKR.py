...
session = requests.Session()
session.verify = False
session.cookies.set("JSESSIONID", "ECF143824B3ED32C3D20BE90E3F4A578.ny5wwwdam1-internet")
...
def check_session():
    ...
    if response.status_code == 200:
        ...
        raise ConnectionError("Session is not active. Please check your connection or session cookie.")
...
def check_market(tickers:list):
    ...
    for symbol in tickers:
        ...
        if data:
            snapshot = data[0]
            print(f"{symbol}: Last={snapshot.get('31')}, Bid={snapshot.get('55')}, Ask={snapshot.get('70')}")
        else:
...
def get_position_quantity(account_id:str, conid:str):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch position for conid {conid} and account {account_id}: {response.status_code} - {response.text}")
    return 0
...
def place_order(symbol:str, quantity:int, side: Literal["BUY", "SELL"]):
    ...
    if not account_id:
        print(f"No active accounts, check your connection.")
        return
    ...
    if side == "SELL":
        ...
        if held < quantity:
            print(f"Not enough holdings to sell. Held: {held}, Tried to sell: {quantity}")
            return
    ...
    response = post_response(f"/api/iserver/account/{account_id}/orders", headers=headers, data=order_payload)
...
def currency_exchange(from_currency:str, to_currency:str, amount:float = 0.0):
    ...
    response = post_response(f"/api/iserver/account/{account_id}/orders", headers=headers, data=payload)
    if response.status_code == 200:
